			...
			stage('Deploy'){
				steps {
					...
					script {
						...
						thistmp = sh(script: 'grep ${prodhost} ~/.ssh/known-hosts|wc -l', returnStdout: true)
						int HostIsKnown = integer.parseint(thistmp)
						thistmp = sh(script: 'grep "$(ssh-keyscan -t rsa ${prodhost})" ~/.ssh/known-hosts|wc -l', returnStdout: true)
